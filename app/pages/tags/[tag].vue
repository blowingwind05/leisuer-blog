...
useHead(() => ({
  title: tagName.value ? `#${tagName.value}` : t('home.tags'),
  meta: [
    {
      name: 'description',
      content: tagName.value ? `#${tagName.value}` : t('home.tags'),
    },
  ],
}))

const { data } = await useAsyncData('tag-posts-' + locale.value + '-' + route.path, async () => {
  ...
  const posts = docs
    .filter((doc) => {
      ...
      return doc.tags?.some(tag => String(tag).trim() === tagName.value)
    })
    .sort((a, b) => {
  ...
  <div class="mx-auto grid w-[min(calc(100%-var(--site-gutter)*2),var(--site-content-width))] grid-cols-1 gap-6 py-10 lg:grid-cols-[minmax(12rem,16rem)_minmax(0,1fr)]">
    ...
    <section class="flex flex-col gap-5">
      <header class="taxonomy-archive-intro-card">
        <p class="taxonomy-archive-eyebrow">#{{ tagName }}</p>
