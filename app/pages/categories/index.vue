...
useHead(() => ({
  ...
  meta: [
    {
      name: 'description',
      content: t('home.categories'),
    },
  ],
}))

const { data: taxonomies } = await useAsyncData('categories-page-taxonomies-' + locale.value, async () => {
...
<template>
  <div class="mx-auto grid w-[min(calc(100%-var(--site-gutter)*2),var(--site-content-width))] grid-cols-1 gap-6 py-10 lg:grid-cols-[minmax(12rem,16rem)_minmax(0,1fr)]">
    ...
    <section class="flex flex-col gap-5">
      ...
      <ContentCategoriesPanel :categories="categories" />
    </section>
  </div>
</template>
